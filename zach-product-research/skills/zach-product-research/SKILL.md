...
description: 基于Sorftime MCP的选品分析，发现高潜力市场机会、多维度属性标注与交叉分析、验证竞争格局、测算投入产出、输出Go/No-Go决策与选品报告。新品上架工作流第一步
...
**下游输出**：选品报告（MD + HTML精简 + Dashboard看板 + Excel）→ 选品报告可作为后续 Listing 优化、定价策略、广告策略的输入
...
- `assets/dashboard_template.html`
  - 用途：Dashboard 可视化看板模板，v2 由 render_deliverables.py 自动注入数据
...
**标注维度**（按品类定制，以下为蓝牙音箱示例；其他品类需根据品类特征自定义维度）：

| 维度 | 解析方法 | 示例规则（蓝牙音箱） |
|------|----------|-------------------|
| 功率 | 正则 `(\d+\.?\d*)\s*[Ww]` | 5W, 20W, 40W, 60W |
| 防水等级 | 正则 `IP[Xx]?\d+` + 关键词 "waterproof", "water resistant" | IPX5, IPX7, IP67, 无 |
| 续航 | 正则 `(\d+\.?\d*)\s*[Hh](ou)?r?s?` + 关键词 "playtime", "battery life" | 8H, 12H, 24H, 30H |
| 产品形态 | 关键词 "portable", "mini", "party", "tower", "soundbar", "shower" | 便携/迷你/派对/塔式/条形/浴室 |
| 灯光 | 关键词 "LED", "RGB", "light", "light show", "ambient" | RGB灯/LED灯/无 |
| 使用场景 | 关键词 "outdoor", "shower", "camping", "party", "home", "travel" | 户外/浴室/露营/派对/家用/旅行 |
...
示例（蓝牙音箱）：功率 x 防水等级、功率 x 产品形态、防水等级 x 使用场景、续航 x 产品形态 等
...
| 要素 | 说明 | 示例（蓝牙音箱-防水维度） |
|------|------|----------------------|
| **痛点描述** | 该维度下消费者最高频的不满 | "号称防水但淋雨后就坏了，实际防水不达标" |
| **数据支撑** | 差评频次/占比 + 涉及竞品 | 28% 差评提及防水不足，涉及 Brand A/B/C |
| **品牌机会** | 品牌在该维度的能力优势 | 品牌有 IPX7 认证工厂资源，可做真实深度防水 |
| **产品方案** | 具体的产品设计方向 | IPX7 认证 + 水下实测视频素材，消除信任疑虑 |
...
适用于用户已确定要分析的产品/品类（如"分析美国站蓝牙音箱市场"、"分析某品类竞争格局"），与场景 1-3 的"发现未知机会"不同。
...
### 品类特定经验：蓝牙音箱（Bluetooth Speaker / Portable Speaker）

| 维度 | 解析要点 | 正则/规则 |
|------|----------|-----------|
| 功率 | 优先匹配明确 W 值；"loud" 不等于高功率；部分标题用 "peak" 功率而非 RMS，需注意区分 | `(\d+\.?\d*)\s*[Ww]` |
| 防水 | IPX 后的数字越大防水越强；"water resistant" ≠ "waterproof"；注意 IP67 同时含防尘等级 | `IP[Xx]?\d+` + 关键词匹配 |
| 续航 | 标题中 "XX hours" 通常指音乐播放时间；"battery life" 和 "playtime" 同义；部分标称值在中等音量下测得 | `(\d+\.?\d*)\s*[Hh]` + "playtime"/"battery" |
| 产品形态 | "portable" 不等于 mini；"party speaker" 通常指大型带灯音箱；"shower speaker" 专指浴室小型款 | 关键词分层匹配 |
| 灯光 | "LED" 单独出现可能是指示灯而非灯光秀；需匹配 "LED light"/"RGB"/"light show" 而非仅 "LED" | 排除仅 "LED" 无上下文的情况 |
...
```
工作成果/brands/{brand}/市场调研/{品类}/{version}/
...
| **消费者痛点** | ⛔ | 差评按维度归类的 Top 5–6 条，每条：标题 + 频率/占比（可选）+ 简短描述 | **痛点卡片网格**（pain-grid + pain-card）：排名圆标 + 标题 + 频率 + desc；可加 insight 框总结品牌切入机会 |
...
| **品牌策略建议** | ⛔ | 产品矩阵或策略建议（可按 Tier1/Tier2 或统一策略表）；规格、价格带、差异化一句 | **策略卡片**（tier-card）或**表格**；与 MD 报告建议一致 |
| **Footer** | ⛔ | 数据来源（Sorftime MCP）、采集日期、完整报告 .md 与 .xlsx 文件名 | 居中、小字、灰色、border-top |

**样式约定**：使用 CSS 变量统一主色与灰度（如 --blue, --gray-100）；容器 max-width 960px、padding 40px 24px；表格带圆角、斑马悬停、表头背景；响应式：小屏 KPI 两列、痛点单列。**参考实现**见 `examples/bluetooth-speaker/` 中的 HTML 精简报告。

**自检**：交付前确认 HTML 含上述 6 个必选区块且无「少量」「约」等模糊表述；与同目录 MD 报告数据一致。

**存储位置**：统一为 `工作成果/brands/[品牌名]/市场调研/[品类slug]/[version]/`；品牌未指定时存于 `工作成果/brands/_选品/市场调研/[品类slug]/[version]/`。不得再写入仓库根目录或旧的 `选品分析/[YYYYMM]` 结构。
...
| **品牌_竞品格局** | 必选 | Top 品牌与竞品角色格局 | 品牌、SKU数、月销量、份额、角色定位、对我方含义 |
...
| **进入壁垒评估** | 必选 | 六类进入壁垒汇总与启动前提 | 壁垒类型、等级、数据锚点、预估成本、预估时间、对我方含义 |
...
1. 按规范目录写入 `工作成果/brands/{brand}/市场调研/{category}/{version}/`
2. v2 生成（4 个交付文件）：
