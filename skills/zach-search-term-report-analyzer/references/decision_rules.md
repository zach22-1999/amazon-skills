...
- 只有在搜索词明显无关、涉及错误人群 / 错误用途 / 错误品类，或已经跨多个观察窗口持续浪费时，才进入 `negative_candidate`。
- 如果词义相关但 `0 单 + 高花费`，优先标记 `reduce_bid` 或 `manual_review`，不要直接否掉；说明应写清是价格、主图、Review、优惠、Listing 承接，还是流量质量导致的问题。
- 对核心品类词、强属性词、强场景词，启动期应先看点击质量、加购 / 转化迹象、Listing 匹配度和竞品价格带，再决定是否降 bid、拆组、转精准或继续观察。
- 当新品缺少 Review、评分、Coupon 或价格优势时，搜索词表现差不一定代表需求不存在，输出中要把“流量无效”和“承接不足”分开写。
- 若广告目标是采集词包或验证定位，允许短期 ACOS 高于目标 ACOS；但必须给出测试截止条件，例如点击量、花费上限、观察天数或最小订单数。
