...
3. ⛔ ASIN 必须是真实可验证的（B0 开头 10 位），严禁使用 B0XXXXX 等占位符
4. ⛔ Review 采集目标不低于 80 条（跨 3-5 个 ASIN），实际采集量写入报告
5. ⛔ Review 原始数据必须走 Python 脚本解析，不能把大段原始 JSON / HTML 直接塞进报告
6. ⛔ 每个 CSV 必须保留可核查来源字段，来源类型必须真实反映数据来源（sorftime_mcp / google_trends / web_search_estimate 等）
7. ⛔ 负面信号必须写入报告
8. ⛔ 所有结论都要引用具体数据，不允许空泛判断
